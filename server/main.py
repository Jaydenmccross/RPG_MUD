        ...
        while player_instance and connection_active:
            ...
            if player_instance.is_dead:
                ...
                if not stripped_msg: # Empty command from alive player
                    # Implement True Player Auto-Attack if in combat and no other action taken
                    if player_instance.in_combat and player_instance.target and \
                       player_instance.is_alive() and not player_instance.has_taken_action_this_turn:
                        ...
                        print(f"[HC_AUTO_ATTACK] Player {player_instance.name} is in combat and entered empty command/timed out. Performing auto-attack on {player_instance.target.name}.")
                        ...
                        if not player_instance.is_alive():
                            print(f"[HC_AUTO_ATTACK_PLAYER_DIED] Player {player_instance.name} died during their auto-attack. Breaking from main loop.")
                            # Death will be handled by the main loop's next iteration checking player_instance.is_dead
                            # or by the finally block if connection_active becomes false.
                            # No 'break' here needed as the outer loop condition will handle it.
                            pass # Allow loop to naturally go to is_dead check
                        elif player_instance.target and not player_instance.target.is_alive():
                            print(f"[HC_AUTO_ATTACK_TARGET_DIED] Target {player_instance.target.name} died from player {player_instance.name}'s auto-attack.")
                            player_instance.add_xp(player_instance.target.xp_value)
                            if player_instance.room:
                                player_instance.room.record_defined_mob_death(player_instance.target)

                            remove_from_active_combat(player_instance.target) # Remove mob

                            original_target_of_player = player_instance.target # Store before clearing
                            player_instance.target = None # Player clears their target

                            # Check if player is still targeted by any other mob
                            still_targeted_by_mob = False
                            with combat_lock:
                                for combatant in ACTIVE_COMBATANTS:
                                    if combatant != player_instance and hasattr(combatant, 'target') and \
                                       combatant.target == player_instance and combatant.is_alive():
                                        still_targeted_by_mob = True
                                        break

                            if not still_targeted_by_mob:
                                print(f"[HC_AUTO_ATTACK] Player {player_instance.name} no longer targeted after killing {original_target_of_player.name if original_target_of_player else 'target'}. Leaving combat.")
                                player_instance.in_combat = False
                                if player_instance in ACTIVE_COMBATANTS: # Player might already be removed if they also died
                                    remove_from_active_combat(player_instance)
                            else:
                                print(f"[HC_AUTO_ATTACK] Player {player_instance.name} killed {original_target_of_player.name if original_target_of_player else 'target'}, but is still targeted by others.")
                        # If both player and target are alive, combat continues. Mob attacks are handled by game_tick.
                    continue # Continue to next iteration of main loop to get new input or prompt
                ...
                if not responded and command_word: # command_word will be non-empty if stripped_msg was non-empty
                    print(f"[DEBUG_HANDLE_CLIENT] Unknown alive command: '{command_word}' from {player_instance.name}")
                    temp_user_for_player.send_message("I don't understand that command.")

            # Check for player death AFTER command processing for the current iteration
            if player_instance and not player_instance.is_alive() and not player_instance.is_dead : # If HP <=0 but is_dead not yet set by handle_death
                print(f"[HC_DIAG_POST_CMD_DEATH_DETECT] Player {player_instance.name} HP is {player_instance.current_hp} after command, but is_dead is {player_instance.is_dead}. Calling handle_death.")
                player_instance.handle_death("Unknown - Post Command Check") # Attacker unknown here
                # The next iteration of the while loop will correctly enter the player_instance.is_dead block
